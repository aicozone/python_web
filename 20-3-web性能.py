...
3. 性能本身评测标准
    ...
    框架作者喜欢测试hello world ？因为hello world本身的压力只有解析请求，路由

4. 如何测试
    ab这个工具 apache benchmark
'''
...
2. 做优化的方法 cache
    a. 有一个操作很耗时，那么我们应该结果cache下来，下一次就很快了
    b. cache肯定不是永久有效的，对于topic来说，你每次改的时候都会失效
    c. 在每次失效的时候更新cache

1. 我们用内存直接cache
2. 我们不用内存，用reids
    以前我们做cache的时候，主要用memcached
    现在redis, 在内存中存储数据
